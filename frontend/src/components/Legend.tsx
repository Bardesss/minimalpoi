...
export default function Legend({ categories, counts, uncategorizedCount = 0 }: { categories: Category[]; counts: Record<number, number>; uncategorizedCount?: number }) {
    ...
    <div style={{ position: "absolute", left: 16, bottom: 16, zIndex: 600, maxWidth: 200, background: "rgba(255,255,255,.94)", backdropFilter: "blur(6px)", border: `1px solid ${theme.color.borderCard}`, borderRadius: theme.radius.card, padding: "12px 14px", boxShadow: theme.shadow.legend }}>
      ...
      {uncategorizedCount > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "2px 0" }}>
          <span style={{ width: 9, height: 9, borderRadius: "50%", background: theme.color.fallbackPin }} />
          <span style={{ fontSize: 12, fontWeight: 600, color: theme.color.textBody }}>Uncategorized</span>
          <span style={{ marginLeft: "auto", fontFamily: theme.font.mono, fontSize: 11, color: theme.color.textInputIcon }}>{uncategorizedCount}</span>
        </div>
      )}
    </div>
  );
