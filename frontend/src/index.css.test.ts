...
// Vite raw import gives the stylesheet text so we can assert the rules exist
// (jsdom does not apply @media CSS to computed styles, so a content assertion
// is the practical regression guard for these CSS-only fixes).
...
describe("index.css mobile ergonomics", () => {
  it("bumps inputs to 16px under the mobile media query (iOS zoom fix)", () => {
    ...
    const has16 = mobileBlocks.some((b) => /font-size:\s*16px\s*!important/.test(b) && /input/.test(b));
